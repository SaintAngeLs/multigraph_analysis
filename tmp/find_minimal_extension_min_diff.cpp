...
struct ElementaryCyclesSearch {
    ...
    bool findCycles(int v, int s, const std::vector<std::vector<int>>& adjList) {
        ...
        for (int i = 0; i < adjList[v].size(); ++i) {
            int w = adjList[v][i];
            if (w == s && stack.size() == adjList.size()) { // force the algorithm to process full cycles
...
int main() {
    ...
    std::vector<std::vector<bool>> adjMatrix(N, std::vector<bool>(N, true)); // create a full graph
